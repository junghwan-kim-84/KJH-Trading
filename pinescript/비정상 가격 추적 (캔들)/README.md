...
- RSI 다이버전스 + 피벗 기준봉 박스
- ATR 배경 구간
- `Strong Long / Strong Short` 진입 후보
...
| Strong L/S | 점수, 거래량, 배경 순서를 합쳐 최종 진입 후보를 만듭니다. | `최소 진입 점수`, `강한 진입 점수`, `총 거래량 MA 기준 봉 수` |
| 유동성 스윕 | 기존 박스를 실제로 쓸고 간 봉과 다음 봉 확인을 표시합니다. | `유동성 스윕 봉 색상 표시`, `스윕 확인 최소 거래량 비율` |
| 박스 유지 | 과거 Strong/다이버전스/스윕 박스를 더 오래 남깁니다. | `과거 박스 모두 유지` |
...
4. `Strong Long / Short`가 뜨면 첫 진입 후보로 해석합니다.
5. 이후 박스를 스윕한 봉이 나오면 다음 봉 다이아몬드로 재확인을 기다립니다.

## 표시 규칙

Strong 진입과 다이버전스는 어두운 톤으로, 스윕 확인은 더 밝은 톤으로 구분해서 겹쳐 보여도 빠르게 식별할 수 있게 설계했습니다.

| 표시 | 의미 |
| --- | --- |
| 주황/하늘 비정상 캔들 | 과열 분출 또는 투매 분출 자리 |
| 어두운 녹색/빨강 동그라미 | `Strong Long / Strong Short` 첫 진입 후보 |
| 어두운 녹색/빨강 박스 | Strong 신호봉 기준 박스 |
...
- Strong Long 박스: `저가 ~ 종가`
- Strong Short 박스: `고가 ~ 종가`
- 강세 다이버전스 박스: `저가 ~ 시가`
- 약세 다이버전스 박스: `고가 ~ 시가`
...
- `Strong Long`은 첫 진입 후보입니다.
- 이후 Strong 박스나 강세 다이버전스 박스를 아래로 한 번 스윕하고, 다음 봉에 밝은 초록 다이아몬드가 나오면 재확인 후보로 봅니다.

### 숏 쪽

- `비정상 과매수 + 약세 다이버전스 + 약세 배경`이 겹치면 상단 분산 후보로 봅니다.
- `Strong Short`는 첫 진입 후보입니다.
- 이후 Strong 박스나 약세 다이버전스 박스를 위로 한 번 스윕하고, 다음 봉에 밝은 빨강 다이아몬드가 나오면 재확인 후보로 봅니다.
...
- Strong 신호도 상위 추세 반대면 짧은 반등/반락으로 끝날 수 있습니다.
