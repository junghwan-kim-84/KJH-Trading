...
- `롱/숏 이벤트 시그널`의 최근 `N봉` 기준 `ON/OFF` 상태 표
- 과거 박스 유지 옵션
...
| 리포트 | 롱/숏 이벤트 시그널을 나열하고, 최근 `N봉` 안에 해당 시그널이 있었는지 `ON/OFF`로 보여줍니다. 기본값은 `20봉`입니다. | `리포트 표시`, `리포트 표시 위치`, `리포트 글자 크기`, `리포트 시그널 체크 봉 수` |
...
6. 마지막으로 `리포트`에서 롱/숏 이벤트 시그널이 각각 최근 `N봉` 안에 있었는지 `ON`/`OFF`로 확인합니다.

## 리포트 읽는 법

리포트는 `최근 N봉 | 롱 | 숏` 3열 구조입니다. 좌상단에는 현재 체크 기준 봉 수가 `최근 20봉`처럼 표시됩니다.

핵심은 지표가 `롱 우세 / 숏 우세`를 대신 판단하는 것이 아니라, 최근 `N봉` 안에 어떤 이벤트 시그널이 있었는지만 바로 확인하는 데 있습니다.

| 항목 | 의미 |
| --- | --- |
| 비정상 시작 | 최근 `N봉` 안에 비정상 과매수/과매도 시작 이벤트가 있었는지 보여줍니다. |
| 비정상 유지 | 최근 `N봉` 안에 비정상 과매수/과매도 캔들이 있었는지 보여줍니다. |
| 다이버전스 | 최근 `N봉` 안에 강세/약세 다이버전스가 있었는지 보여줍니다. |
| ATR 배경 | 최근 `N봉` 안에 ATR 배경 구간이 있었는지 보여줍니다. |
| ATR 강화 | 최근 `N봉` 안에 강한 ATR 배경까지 붙었는지 보여줍니다. |
| 오더 블럭 | 최근 `N봉` 안에 롱/숏 진입 후보 이벤트가 있었는지 보여줍니다. |
| 스윕 확인 | 최근 `N봉` 안에 스윕 뒤 확인 다이아몬드 이벤트가 있었는지 보여줍니다. |

표 안의 값은 단순합니다.

- `ON`: 해당 방향 시그널이 최근 `N봉` 안에 있었음
- `OFF`: 해당 방향 시그널이 최근 `N봉` 안에 없었음
...
- 리포트는 포지션 판단을 대신하지 않고, 최근 `N봉` 기준 이벤트 시그널 상태만 보여주는 보조 표입니다.
